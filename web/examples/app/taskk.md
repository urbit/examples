...
We use clay as our shared state. This allows multiple users to sync issues, 
etc. The basic unit of the app is the issue, which is stored as an md file. 
As much as possible is encoded in the file path, so that app functions as 
a wrapper around clay to enforce rules, etc.

Any large-scale edits can be done in vim or a similar editor by simply editing the 
markdown file associated with an issue.
...
## Instructions 
You can also interact with the taskk app from the `:dojo` by poking the app 
with the appropriate marks. The following marks are available.
