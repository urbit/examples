...
/-    examples-message                         ::  import message structure
::
::
...
|_  $:  hid/bowl                      ::  system data
      sent/(map @da examples-message)         ::  app state data structures
    received/(map @da examples-message)
    ==
...
++  peer                              ::  get 1st time subs. add them to sup
  ...
  =+  ^=  ordered-messages            ::  push on ordered list of all messages
      ...
      |=  $:  a/(pair @da examples-message)       ::  comparing by time
          b/(pair @da examples-message)
          ==
      (gth p.a p.b)
...
++  inbox-to-json                     ::  convert received messages to ++json
  |=  messes/(list {@da examples-message})    ::  accept map of time to message
  :-  %a  ^-  (list json)             ::  compose json array of msgs
  %+  turn  messes                    ::  map over listified map of @da to msg
  |=  {tym/time mez/examples-message}          ::  func that makes json msgs
...
++  msg-to-json                       ::  convert msgs to json
  |=  mez/examples-message                     ::  accept msg
...
++  poke-examples-message                      ::  receive data with mark of "message"
  |=  {mez/examples-message}
  ...
  =+  ^=  ordered-msgs                ::  push on sorted list of messages
      ...
      |=  {a/(pair @da examples-message) b/(pair @da examples-message)} ::
